...
class LogWatcher(object):
        ...
        for id, file in self._files_map.items():
            ...
            if tail_lines:
                try:
                    ...
                    if lines:
                        self._callback('{0}:{1}'.format('i', file.name), lines)
    ...
    def open(cls, file):
        ...
        """
        import codecs, locale
        return codecs.open(file, 'r', encoding=locale.getpreferredencoding(), errors='ignore')
        # return open(file, 'rb')
    ...
    def readlines(self, file):
        ...
        while True:
            ...
            if not lines:
                break
            self._callback('{0}:{1}'.format('R', file.name), lines)
    ...
    def unwatch(self, file, fid):
        ...
        with file:
            ...
            if lines:
                self._callback('{0}:{1}'.format('U', file.name), lines)
